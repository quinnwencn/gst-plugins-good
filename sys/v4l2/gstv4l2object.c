...
#define ENCODED_BUFFER_SIZE             (6 * 1024 * 1024)
    ...
    gboolean try_only, GstV4l2Error * error)
{
  ...
  if (V4L2_TYPE_IS_OUTPUT (v4l2object->type) && (pixelformat == V4L2_PIX_FMT_HEVC
...
gst_v4l2_object_decide_allocation (GstV4l2Object * obj, GstQuery * query)
{
  ...
  alloc_has_meta = gst_query_find_allocation_meta (query, GST_DMABUF_META_API_TYPE,
      &alloc_index);
  ...
  if (IS_IMX8MQ() || IS_AMPHION()) {
    if (alloc_has_meta) {
      ...
      GST_DEBUG_OBJECT (obj->dbg_obj, "Expected field 'GstDmabufMeta' in structure: %"
          GST_PTR_FORMAT, params);
      if (params) {
        ...
        if (GST_VALUE_HOLDS_LIST (vdrm_modifier)) {
          ...
          for (j = 0; j < len; j++) {
            ...
            GST_DEBUG_OBJECT (obj->dbg_obj, "dmabuf meta has modifier: %lld", drm_modifier);
            if (IS_AMPHION() && drm_modifier == DRM_FORMAT_MOD_AMPHION_TILED) {
              obj->drm_modifier = drm_modifier;
            } else if (IS_IMX8MQ() && drm_modifier == DRM_FORMAT_MOD_VSI_G2_TILED_COMPRESSED
                  && obj->is_g2 == TRUE) {
                obj->drm_modifier = drm_modifier;
            ...
            GST_DEBUG_OBJECT (obj->dbg_obj, "dmabuf meta has modifier: %lld", drm_modifier);
            if (IS_AMPHION() && drm_modifier == DRM_FORMAT_MOD_AMPHION_TILED) {
              GST_DEBUG_OBJECT (obj->dbg_obj, "video sink support modifier: %lld", drm_modifier);
              obj->drm_modifier = drm_modifier;
            } else if (IS_IMX8MQ() && drm_modifier == DRM_FORMAT_MOD_VSI_G2_TILED_COMPRESSED
                  && obj->is_g2 == TRUE) {
                GST_DEBUG_OBJECT (obj->dbg_obj, "video sink support modifier: %lld", drm_modifier);
        ...
        if (obj->ioctl (obj->video_fd, VIDIOC_S_FMT, &obj->format) < 0 )
          ...
          GST_DEBUG_OBJECT (obj->dbg_obj, "Set tiled output format %" GST_FOURCC_FORMAT,
