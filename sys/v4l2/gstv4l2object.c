...
gst_v4l2_object_v4l2fourcc_is_codec (guint32 fourcc)
{
  ...
  for (i = 0; i < GST_V4L2_FORMAT_COUNT; i++) {
    if (gst_v4l2_formats[i].format != fourcc)
      continue;
    ret = gst_v4l2_formats[i].flags & GST_V4L2_CODEC;
  }
    ...
    struct v4l2_format *fmt, GstVideoColorimetry * cinfo)
{
  ...
  is_codec = gst_v4l2_object_v4l2fourcc_is_codec (pixelformat);
  ...
  if (is_rgb || (is_codec && v4l2object->matrix == GST_VIDEO_COLOR_MATRIX_RGB))
    ...
    gboolean try_only, GstV4l2Error * error)
{
  ...
  if (gst_v4l2_object_get_interlace_mode (format.fmt.pix.field,
    ...
    if (gst_structure_has_field (s, "interlace-mode")) {
      if ((format.fmt.pix.field == V4L2_FIELD_SEQ_TB) && (field == V4L2_FIELD_INTERLACED)) {
        GST_DEBUG_OBJECT (v4l2object->dbg_obj, "bypass field check if field from driver: %s and current field: %s",
