...
gst_v4l2_video_dec_setup_capture (GstVideoDecoder * decoder)
{
  ...
  if (G_UNLIKELY (!GST_V4L2_IS_ACTIVE (self->v4l2capture))) {
    ...
    if ((self->v4l2capture->format.fmt.pix.xfer_func == V4L2_XFER_FUNC_SMPTE2084
            ...
            V4L2_XFER_FUNC_HLG)
        && info.finfo->format == GST_VIDEO_FORMAT_NV12_10LE) {
      ...
      if (output_state->caps) {
        ...
        gst_video_mastering_display_info_add_to_caps (&minfo,
            output_state->caps);
  ...
  if (g_atomic_int_get (&self->capture_configuration_change)) {
    ...
    if (ret != GST_FLOW_OK) {
      ...
      if (ret == GST_FLOW_NOT_NEGOTIATED) {
        GST_ERROR_OBJECT (decoder, "capture configuration change fail, return negotiation fail");
        goto beach;
      } else {
        return;
      }
